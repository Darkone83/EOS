...
//
// 'stall' -- byte-granularity backpressure for whoever consumes dvalid/dout
// during a block read (ST_RD_DATA/ST_RD_CRC only). Holding stall high simply
// delays the start of the NEXT byte; the SD SPI clock just pauses between
// bytes, which is always safe (static interface, same as eos_flash_reader.v's
// stall note). The one byte already in flight always completes normally.
//
...
module eos_sd_spi #(
    ...
    input  wire        stall,        // hold DATA/CRC at a byte boundary (see below)
    output reg         busy,
    ...
    always @(posedge clk or negedge rstn) begin
        if (!rstn) begin
            ...
            case (st)
                ...
                ST_RD_DATA: begin
                    if (!byte_go && !b_busy && !byte_done_r && !stall) begin
                        tx_byte <= 8'hFF; byte_go <= 1'b1;
                    end
                    if (byte_done_r) begin
