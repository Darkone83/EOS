...
//   27 MHz sys_clk -> 64.8 MHz (x12 / 5) on CLKOUT, plus a phase-shifted
//   copy on CLKOUTP for the controller's clk_sdram capture clock.
//   VCO = 64.8 * ODIV(8) = 518.4 MHz (in range). PSDA_SEL="1010" = 225 deg.
//
...
// Generator > rPLL (CLKIN 27 MHz, CLKOUT 64.8 MHz, enable CLKOUTP with the selected phase) and drop
...
module eos_sdram_pll (
    ...
    output clkoutp,    // 64.8 MHz, 225 deg -> sdram.clk_sdram
    output lock
);
    rPLL u_pll (
    ...
    defparam u_pll.PSDA_SEL        = "1010";   // CLKOUTP shifted 225 deg
